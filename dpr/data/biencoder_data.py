...
class T2RankingDataset(Dataset):
    ...
    def _load_all_data(self):
        logger.info("Query files: %s", self.query_file)
        logger.info("Collection files: %s", self.collection)
        logger.info("Neg files: %s", self.neg_file)
        logger.info("Reading collection file")
        self.collection = pd.read_csv(self.collection,sep="\t", quoting=3)
        ...
        self.query = pd.read_csv(self.query_file,sep="\t")
        self.query.columns = ['qid','text']
        self.query.index = self.query.qid.astype(int)
        self.query.pop('qid')
        logger.info("Reading neg file")
        top1000 = pd.read_csv(self.neg_file, sep="\t")
        if len(top1000.columns)==3:
            top1000.columns=['qid','pid','index']
        elif len(top1000.columns)==4:
            top1000.columns=['qid','pid','index','score']
        else:
            print("Wrong negative samples file format, length of columns is ", len(top1000.columns))
            return
        self.data = list(top1000.groupby("qid"))
        qrels={}
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        cols = self.data[index]
        qid = cols[0]
        neg_pids = cols[1]["pid"].astype(int).tolist()
        pos_ids = self.qrels.get(qid)
        query = self.query.loc[qid]['text']
        r = BiEncoderSample()
        r.query = self._process_query(query)
        positive_ctxs = []
        for pos_id in pos_ids:
            data = self.collection.loc[pos_id]
            positive_ctxs.append({"text": data['para']})
        hard_negative_ctxs = []
        for neg_pid in neg_pids:
            data = self.collection.loc[neg_pid]
            hard_negative_ctxs.append({"text": data['para']})
...
class TsvQADataset(Dataset):
    ...
    def _load_all_data(self):
        ...
        #从tsv文件读取, 每个tsv文件每行有一个query, 一个positive_ctx, 一个hard_negative_ctx
        data = []
        for path in self.data_files:
            with open(path, 'r') as f:
                ...
                for line in lines:
                    ...
                    if len(line) != 4 or line[-1] != "0":
                        print("TSV data format wrong")
                        return
                    if data and data[-1]["question"] == line[0]:
                        if {"text":line[1]} not in data[-1]["positive_ctxs"]:
                            data[-1]["positive_ctxs"].append({"text":line[1]})
                        if {"text":line[2]} not in data[-1]["hard_negative_ctxs"]:
                            data[-1]["hard_negative_ctxs"].append({"text":line[2]})
                    else:
                        data.append({"question":line[0],"positive_ctxs":[{"text":line[1]}],"hard_negative_ctxs":[{"text":line[2]}]})
...
class Jsonlist_normal_QADataset(Dataset):
    def __init__(
        self,
        file: str,
        selector: DictConfig = None,
...
class JsonlistQADataset(Dataset):
    def __init__(
        self,
        file: str,
        selector: DictConfig = None,
    ...
    def _load_all_data(self):
        """
        仿照上方的JsonQADataset类的_load_all_data()方法，从你的文件中读取数据到self.data
        self.file即为你在train_encoder.sh中配置的训练集和测试集数据路径
        """
        self.data_files = get_dpr_files(self.file)
        logger.info("Data files: %s", self.data_files)
        data = read_data_from_json_files(self.data_files)
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        """
        仿照上方的JsonQADataset类的__getitem__()方法，从self.data中读取数据到BiEncoderSample对象
        """
        json_sample = self.data[index]
        r = BiEncoderSample()
        r.query = self._process_query(json_sample["question"])
