...
class BiEncoderTrainer(object):
    ...
    provided pools of negative passages.
    """
    ...
    def __init__(self, cfg: DictConfig):
        ...
        # 从checkpoint路径中获取已经训练过的model，若checkpoint_file_name不为合法路径，则model_file为None
        model_file = get_model_file(cfg, cfg.checkpoint_file_name)
        saved_state = None
        if model_file:
            # 从checkpoint文件中获取模型参数
            logger.info("Find checkpoint, Loading saved model from model file ...")
            saved_state = load_states_from_checkpoint(model_file)
            set_cfg_params_from_state(saved_state.encoder_params, cfg)
        # 以下函数跳转至./dpr/models/hf_models get_bert_biencoder_components
        tensorizer, model, optimizer = init_biencoder_components(cfg.encoder.encoder_model_type, cfg)
        model, optimizer = setup_for_distributed_mode( # 分布式训练相关, 可以忽略
            model,
            optimizer,
            cfg.device,
            cfg.n_gpu,
    ...
    def run_train(self):
        cfg = self.cfg
        train_iterator = self.get_data_iterator( # 获取数据集迭代器
        ...
        if self.scheduler_state: # 学习率规划相关, 可以忽略
    ...
    def validate_and_save(self, epoch: int, iteration: int, scheduler):
        ...
        if not cfg.dev_datasets:
            ...
            if epoch >= cfg.val_av_rank_start_epoch:
                validation_loss = self.validate_average_rank() # 这种验证方式可以忽略
            else:
                validation_loss = self.validate_nll() # 计算模型在dev集上的表现

        if save_cp:
            cp_name = self._save_checkpoint(scheduler, epoch, iteration)
            logger.info("Saved checkpoint to %s", cp_name)

            if validation_loss < (self.best_validation_result or validation_loss + 1): # 记录表现最好的checkpoint
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            if cfg.fp16: # 精度相关, 可以忽略
                from apex import amp
                with amp.scale_loss(loss, self.optimizer) as scaled_loss:
                    scaled_loss.backward() # 反向传播, 更新模型参数
            ...
            if data_iteration % eval_step == 0:
                ...
                self.validate_and_save(epoch, train_data_iterator.get_iteration(), scheduler) # 在dev集上测试效果, 并保存checkpoint
                self.biencoder.train()
    ...
    def _load_saved_state(self, saved_state: CheckpointState):
        ...
        epoch += 1
        offset = 0
        logger.info("Continue Training @ batch=%s and epoch=%s", offset, epoch)
...
def main(cfg: DictConfig):
    ...
    cfg = setup_cfg_gpu(cfg) # 分布式训练相关, 可以忽略
    set_seed(cfg)
    ...
    trainer = BiEncoderTrainer(cfg) # 该类封装了model, optimizer, tensorizer以及dataset等所有训练相关的内容
